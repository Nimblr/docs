...
--- | --- | --- | --- | ---
`selfEmail` | true | string | The email/id from the calendar owner. It is not easy to obtain it via OAuth. As soon as a new appt is created, we see and grab it. | 256
`name` | true | string | Calendar Name. It normally comes from the provide's name (i.e. Google calls it `summary`) | 128
...
name | required	|	type	|	description	|	length
---	|	---	|	---	|	--- | ---
`externalId` | true | string	|	Reference to the original calendar event ID (For example, Google uses a longish UUID)	|	
`prefix`	| false | string	|	A small prefix with event status (icons that reflect progress)	|	16
`summary`	| false | string	|	Event summary shown as in the event title	|	
`type`  	| false | string	|	Type of event. It's calendar provider specific. It may not exist for some calendars (i.e. Google/Outlook). It can also be appt type (i.e. 'First Visit') or 'BLOCK' for blocking slots	|	
...
`color`	| fasle |	string	|	Event color. Note that the event colors used for now are: green, yellow and red. Each implementation will map the colors accordingly	|	16
`description`	| false |	string	| Full description of the meeting	|	
`organizer`	| true |	string	|	Email/ID of the organizer (it's the Owner of the calendar)	|	
`creator`	| true |	string	|	Email/ID of the creator (if the calendar has been delegated, the creator will be the person who created the event)	|	
`start`	| true |	date	|	DateTime when the appt starts in UTC	|	
`end`	| true |	date	|	DateTime when the appt ends in UTC	|	
`allDay`	| false | boolean	|	This is a one (or multiple) days with time duration of a complete day, Default *false*	|	
`deleted`	| true	| boolean	|	True if the event has been deleted or cancelled by the organizer	|	
`timeZone`	| false	| string	|	Time zone according to the IANA Time Zone Database name. If it is not set, the calendar time zone will be used |
`contacts`	| true	| object	|	Map from email to contact (if embedded in the event itself)	|	
...
`address`	| false	| string	|	 Event's address. Allows to override preferences and calendar address	|	
`phone`	| false	| string	|	Specific phone of the event. Allows to override preferences and calendar phone	|	
`url`	| false	| string	|	Contains the url for a telemedicine appt|
...
`room`  | false | String | Room identifier for the event (only for providers handling rooms) |
...
`language` | false |	string	|	Language spoken by user in two letter codes (ISO639-1)	|	
`created`	| true	| date	|	DateTime when the contact was created	|	
`modified`	| true | date	|	DateTime when the contact was created	|	
